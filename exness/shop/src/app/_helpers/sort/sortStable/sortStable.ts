...
export function sortStable<T>(source: T[], key: keyof T, direction: SortDirection): T[] {
    const comparator =
        direction === SortDirection.asc ? ascComparatorFactory(key) : descComparatorFactory(key);

    const stabilized = source.map((el, index) => <[T, number]>[el, index]);
    const stableCmparator: ISortStableComparator<[T, number]> = (a, b) => {
    ...
    stabilized.sort(stableCmparator);
